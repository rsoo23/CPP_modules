...
/*   Updated: 2023/12/06 18:31:00 by rsoo             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void BitcoinExchange::parse_input_file( char* infile_name ) {
	...
	while (std::getline(inputFile, line)) {
		...
		if (check_valid_date(date) && check_valid_val(val)) {
			std::map<std::string, float>::iterator pair = _btcDataMap.lower_bound(date);
			float res = pair->second * _value;

			std::cout << std::setprecision(3) << GREEN << _lineNum++ << ": " << date << " => " << pair->second << " * " << val << " = " << res << std::endl;
		}
	}
}
